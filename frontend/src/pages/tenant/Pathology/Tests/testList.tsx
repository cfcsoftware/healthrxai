...
const PathologyTestList = () => {
  ...
  useEffect(() => {
    const fetchTests = async () => {
      try {
        const res = await axiosTenant.get("/pathology/test/list?api=true");
  ...
  const handleExportJSON = () => {
    exportToJSON("pathology_tests.json", filteredTests);
  };

  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
        <h1 className="text-3xl font-extrabold text-gray-900">Pathology Test List</h1>
