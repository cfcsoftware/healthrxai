  ...
  const PathologyReportList = () => {
    ...
    useEffect(() => {
      const fetchReports = async () => {
        try {
          const res = await axiosTenant.get("/pathology/report/list?api=true");
          setReports(res.data.data);
        } catch (err) {
          console.error("Error fetching pathology reports", err);
        } finally {
          setLoading(false);
        }
      };
    ...
    const handleExportJSON = () => {
      exportToJSON("pathology_reports.json", filteredReports);
    };
  
    return (
      <div className="min-h-screen bg-gray-50 p-6 md:p-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
          <h1 className="text-3xl font-extrabold text-gray-900">Pathology Reports</h1>
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {paginatedReports.map((r, index) => (
                  <tr key={r.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">{r.short_name}</td>
                    <td className="px-6 py-4">{r.sample_type}</td>
                    <td className="px-6 py-4 text-green-700 font-semibold">
                      ₹{r.total_amount}
                    </td>
                    <td className="px-6 py-4">{r.precaution}</td>
